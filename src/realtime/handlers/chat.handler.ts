...
export function registerChatHandlers(io: Server, socket: Socket) {
  ...
  socket.on('SEND_MESSAGE', async (payload: { conversationId: string; text: string; targetUserId: string }) => {
    try {
      ...
      if (sender.blockedUsers?.includes(targetUserId) || target.blockedUsers?.includes(userId)) {
        logger.warn({ userId, targetUserId }, 'SEND_MESSAGE blocked by block system');
      ...
      const targetRoom = io.sockets.adapter.rooms.get(targetUserId);
